...
class DataRepository(Repository):
    ...
    def save(self, request: SaveRequest) -> None:
        ...
        directory = self.root_dir / request.layer.value / request.provider
        ...
        if request.layer == DataLayer.RAW:
            ...
            with open(file_path, "wb") as file:
                file.write(orjson.dumps(request.payload, option=orjson.OPT_INDENT_2))
    ...
    def load(self, layer: DataLayer, provider: DataSource, key: str) -> dict[str, Any]:

        file_path = self.root_dir / layer.value / provider.value / f"{key}.json"
    ...
    def exists(self, layer: DataLayer, provider: str, key: str) -> bool:
        ...
        return (self.root_dir / layer / provider / f"{key}.json").exists()

    def delete(self, layer: DataLayer, provider: str, key: str) -> None:

        file_path = self.root_dir / layer / provider / f"{key}.json"
    ...
    def list_providers(self, layer: DataLayer) -> list[DataSource]:
        ...
        for item in folders.iterdir():
            ...
            try:
                providers.append(DataSource(item.name))
    ...
    def list_keys(
        ...
        for provider in providers:
            folder = self.root_dir / layer / provider.value

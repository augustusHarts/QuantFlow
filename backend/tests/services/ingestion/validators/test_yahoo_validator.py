...
def test_result_not_dict(
    ...
    with pytest.raises(
        YahooInvalidResponseError,
        match="result must contain dictionaries"
    ):
        validator.validate(payload)
...
def test_invalid_timestamp_type(
    ...
    with pytest.raises(
        YahooInvalidResponseError,
        match="Timestamp must contain integers"
    ):
        validator.validate(payload)
...
def test_indicators_not_dict(
    ...
    with pytest.raises(
        YahooInvalidResponseError,
        match="indicators must be a dictionary"
    ):
        validator.validate(payload)
...
def test_quote_not_dict(
    ...
    with pytest.raises(
        YahooInvalidResponseError,
        match="Quote must contain a dictionary"
    ):
        validator.validate(payload)
...
def test_required_field_not_list(
    validator,
    valid_payload,
    field
):
...
def test_invalid_price_type(
    ...
    with pytest.raises(
        YahooInvalidResponseError,
        match=f"{field} contains invalid values"
    ):
        validator.validate(payload)
...
def test_invalid_volume_type(
    ...
    with pytest.raises(
        YahooInvalidResponseError,
        match="volume contains invalid values"
    ):
        validator.validate(payload)
...
def test_adjclose_item_not_dict(
    validator,
    valid_payload
):
    payload = deepcopy(valid_payload)
    ...
    with pytest.raises(
        YahooInvalidResponseError,
        match="adjclose must contain a dictionary"
    ):
        validator.validate(payload)

def test_adjclose_values_not_list(
    validator,
    valid_payload
):
    payload = deepcopy(valid_payload)
...
def test_adjclose_length_mismatch(
    validator,
    valid_payload
):
    payload = deepcopy(valid_payload)
    ...
    with pytest.raises(
        YahooInvalidResponseError,
        match="adjclose length mismatch"
    ):
        validator.validate(payload)

def test_adjclose_invalid_type(
    validator,
    valid_payload
):
    payload = deepcopy(valid_payload)
